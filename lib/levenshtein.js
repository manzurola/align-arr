...
const defaultEquals = (s, t) => s === t;
const levenshtein = (s, t, { delCost = defaultCost, insCost = defaultCost, subCost = defaultCost, equals = defaultEquals, } = {}) => {
    const sLen = s.length;
    const tLen = t.length;
...
const initMatrix = (sLen, tLen) => {
    const matrix = [...Array(sLen + 1)].map(() => [...Array(tLen + 1)].map(() => ({
        cost: 0.0,
        opCost: 0.0,
    })));
    // Fill in the edges
